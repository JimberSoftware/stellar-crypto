...
describe('funding', () => {
    it('should check an account for vesting acccount', async () => {
        ...
        expect(account.account_id).toEqual(
            'GBD5VSSCS2W3VFBLGV4GS4JNKFE2X7V5U6CWT3JDLGHJDT4I6W4LCCXL'
        );
    }, 60000);

    it('should not find vesting acccount for random address', async () => {
        const account = await checkVesting(
            'GDSMZD335AGTDJ667PXDKHQHHLMGN7DXMGX6BC5VUPCPBECDJJWO2YFX'
        );
    ...
    it('should generate a vesting account', async () => {
        ...
        expect(account.signers).toHaveLength(11);
        const mySigner = account.signers.find(
            signer => signer.key === randomKeypair.publicKey()
        );
        expect(mySigner.weight).toBe(5);
        const ownSigner = account.signers.find(
            signer => signer.key === account.id
        );
        expect(ownSigner.weight).toBe(0);

        const otherSigners = account.signers.filter(
            signer =>
                signer.key !== randomKeypair.publicKey() &&
